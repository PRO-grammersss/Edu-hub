...
def education_policy(request):
    if not request.user.is_authenticated or request.user == 'AnonymousUser': #User is not authenticated or not signed in 
        return render(request, "ccr/login.html", { #Redirects the user to the login page if they try to access the home page without signing in 
            'message': 'Sign in first'
        }) 
    else: #If user is authenticated and not anonymous 
        return render(request, "ccr/education_policy.html") #Sends the user to the education policy page.

def about(request):
    if not request.user.is_authenticated or request.user == 'AnonymousUser': #User is not authenticated or not signed in 
        return render(request, "ccr/login.html", { #Redirects the user to the login page if they try to access the about page without signing in 
            'message': 'Sign in first'
        }) 
    else: #If user is authenticated and not anonymous 
        return render(request, "ccr/about.html") #Sends the user to the about page.
